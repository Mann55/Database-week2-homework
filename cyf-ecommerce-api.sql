...
----- get all the products  to load all the product names along with their supplier names.
...
select *  from customers c ORDER BY c."name"  DESC;
...
inner join suppliers s on s.id = p.id 
inner join orders o on o.id = s.id 
inner join customers c on c.id = o.customer_id 
inner join order_items oi on oi.order_id = c.id;
